...
const CrudClasseL2 = () => {
  ...
  //     })
  //     .catch((error) =>
  //       console.error("Error fetching data student level 2:", error)
  //     );
  // };

  const fetchClasseL1 = useCallback(async () => {
    try {
      const response = await api.get("/curentListStudent/niveau/licence2");
      ...
      // setData(sortedData.reverse()); // Inverser l'ordre des données
      setData(sortedData); // Stocke les données triées
    } catch (error) {
      console.error(
        "Error fetching data student level 2:",
  ...
  const handleAdd = async (newData) => {
    try {
      ...
      if (response.status === 201) {
        // Code 201 pour une création réussie
        const data = response.data; // Axios renvoie déjà les données en JSON
        setIsAddModalOpen(false);
        addForm.resetFields();
        setData((prevData) => [data, ...prevData]); // Ajouter le nouvel élément au début
      } else {
